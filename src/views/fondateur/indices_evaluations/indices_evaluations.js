...
const ListIndices = memo((props) => {
  ...
  const submitForm = () => {
    http.post("/indices", inputs).then((res) => {
      alert("L'indice d'évaluation a été ajouté avec succès !");
  ...
  const deleteIndices = (id) => {
    if(window.confirm("Voulez-vous supprimer cet élément?") == true){
    http.delete("/del_indices/" + id).then((res) => {
      fetchAllIndices();
    });
      alert('Supprimé!');
  };

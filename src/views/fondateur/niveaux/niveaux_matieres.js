...
const NiveauxMatieres = memo((props) => {
    ...
    const handleChange = (event) => {
        ...
        setInputs(values => ({ ...values, [name]: value, etab, niveau }))
    }
    ...
    const submitForm = () => {
        http.post('/niveauxmatieres', inputs).then((res) => {
            alert("Matière ajoutée avec succès !")
            navigate('/List/matieres/niveaux/' + niveau)
    ...
    const fetchAllmatieres = () => {
        http.get('/get_all_matieres_for_niveau/' + etab).then(res => {
            setmatieres(res.data);
        })
    }

    const [matieres_niveau, setmatieres_niveau] = useState([]);
    useEffect(() => {
        fetchAllmatieres_niveau();
    }, []);

    const fetchAllmatieres_niveau = () => {
        http.get('/get_matieres_for_niveau/' + etab + '/' + niveau).then(res => {
            setmatieres_niveau(res.data);
        })
    }
                                            ...
                                            <tr key={item.id}>
                                                ...
                                                <td>
                                                    <div className="flex align-items-center list-user-action">
                                                        ...
                                                        <Link className="btn btn-sm btn-icon btn-danger" data-toggle="tooltip" data-placement="top" title="Delete" data-original-title="Delete" to="#" onClick={() => {delete_niveau_matiere(item.id)}}>
                                                            <span className="btn-inner">

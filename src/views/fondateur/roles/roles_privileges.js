...
const RolesPrivilegesAdmin = memo((props) => {
  ...
  const fetchAllPrivileges = () => {
    http.get("/get_privileges").then((res) => {
  ...
  const handleChange = (event) => {
    ...
    setInputs((values) => ({ ...values, [name]: value, etab, id_role }));
  };
  ...
  const fetchAllprivilegeRole = () => {
    http.get("/get/privileges/role/" + etab + "/" + id_role).then((res) => {
      setprivilegeRole(res.data);
    });
  };
                          ...
                          <Form.Group className="form-group">
                            ...
                            >
                              ...
                              {privileges.map((item) => (
                                <option
                                  key={item.id}
                                  value={item.code_privilege}
                                >
                                  {item.intitule_privilege}
                                </option>
                              ))}
                            </select>
                          </Form.Group>
            ...
            <Card.Body>
              <div className="table-responsive border-bottom my-3">
                ...
                >
                  ...
                  <tbody>
                    {privilegeRole.map((item, idx) => (
                      <tr key={idx}>
                        <td>{item.code_privilege}</td>
                        <td>{item.intitule_privilege}</td>
                        <td>{item.type_privilege}</td>
                        <td>
                          <div className="flex align-items-center list-user-action">
                             <Link className="btn btn-sm btn-icon btn-danger" data-toggle="tooltip" data-placement="top" title="Delete" data-original-title="Delete" to="#" onClick={() => {
                                                            deletePrivileges_roles(item.code_privilege);
                                                        }}>
                                                            <span className="btn-inner">
...
export default RolesPrivilegesAdmin;

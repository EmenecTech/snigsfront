...
const [nbretab_sup, setnbretab_sup] = useState([]);
  ...
  const [nbretap, setnbretap] = useState([]);
  useEffect(() => {
    fetchAllnbretap();
  }, []);

  const fetchAllnbretap = () => {
    http.get('/nbr/etab/' + etab).then(res => {
      setnbretap(res.data);
    })
  }


  const [nbrparsup, setnbrparsup] = useState([]);
    useEffect(() => {
        fetchAllnbrparsup();
    }, []);

    const fetchAllnbrparsup = () => {
        http.get('/nbreleve_parent_sup/' + etab).then(res => {
            setnbrparsup(res.data);
        })
    }
                      ...
                      <div className="card-body">
                        <div className="progress-widget">
                          ...
                          <div className="progress-detail">
                            ...
                            <h4 className="counter">
                              <CountUp start={0} end={sumpay} duration={5} /> FCFA
                            </h4>
                          </div>
                        </div>
                      </div>
